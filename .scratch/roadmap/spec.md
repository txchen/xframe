...
Updated: 2026-09-22. Original baseline: `1944c38`. Console implementation: `eba77e2`.
...
- Live single-controller integration is implemented behind View > Enable Controller Input; Palworld controls and focus-loss release have passed user-observed physical acceptance. The user also reported normal Palworld rumble on xCloud after the duration fix. Series X and other-controller rumble still need physical testing.
- Current implementation passes 154 tests and the signed app build verifies. The [console validation record](../xbox-console-remote-play/validation.md) separates automated evidence from pending Series X acceptance; individual feature documents retain their own live-validation limits.

## Priority and completion criteria

### P0 — Complete the playable xCloud loop

- [x] **Basic controller integration — Xbox One Bluetooth.** Accepted by the user on 2026-09-22 after Palworld tests covering controls, focus release, reconnect, fullscreen and repeated-session input. PS4 / DualShock 4 and other controllers are **not tested**. Abrupt battery-removal release delay remains a [known limitation](../controller-input/issues/02-abrupt-power-loss-delay.md); the user later reported normal xCloud Palworld rumble after the duration fix, while Series X and other-controller rumble remain unverified. Input remains opt-in via View > Enable Controller Input; Controller Vibration can be turned off independently. [Validation](../controller-input/validation.md), [spec](../controller-input/spec.md).
...
| LAN Xbox console streaming | Code committed; physical acceptance pending | `eba77e2` adds listing, wake then connect, direct-path enforcement, shared media/input and cleanup. Series X media, controls, rumble and recovery still require live testing. |

Latest full suite: **154 tests passed**; signed app build verified. Controller enable/disable preferences persist across launches; focus ownership and held inputs do not. [Earlier live reliability evidence](../stream-reliability/live-validation-2026-09-22.md). Console code is committed in `eba77e2`; its physical acceptance remains open.

**Earlier increment: [1080p → 4K MetalFX Spatial](../4k-post-processing/issues/01-spatial-upscaling.md) is implemented.** Original / Integer Scaling / MetalFX Spatial switch during playback and persist across launches. At that increment, the signed build and 133 tests passed; all three reached 60 fps recent output in a matched M1/4K Palworld menu comparison. MetalFX increased GPU and observed presentation cost. The user reports substantially better MetalFX quality and sharp Integer Scaling text; mild MetalFX motion artifacts are accepted as a quality observation. Extended A/V and settings-controller acceptance remain; sharpening and alternate backends follow separately. [Validation](../4k-post-processing/spatial-validation.md). Continue reliability soak/long-outage acceptance as separately tracked work; beginning P1 does not mark P0 complete.
...
- [ ] **Xbox Console Remote Play — code complete, awaiting Series X acceptance.** Commit `eba77e2` adds account-separated xHome access, associated-console listing and LAN discovery, explicit wake then connect, shared media/input and session cleanup, LAN ICE enforcement, and controller rumble. All 154 automated tests, release app build, and signing checks pass. The checkbox remains open until live Series X video/audio/input, physical rumble, disconnect behavior, and recovery are verified. See [spec](../xbox-console-remote-play/spec.md) and [validation](../xbox-console-remote-play/validation.md).
...
Other-controller validation, abrupt power-loss detection delay, physical rumble acceptance and multi-controller support remain follow-ups rather than additional top-level checkbox items. Low latency live performance validation remains pending. The earlier planning baseline was `b77f40c` and planning commit `57fd75f`; console implementation is committed in `eba77e2` with 154 passing tests and a verified signed build. Series X acceptance is still pending.
