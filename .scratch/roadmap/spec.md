...
- Current implementation passes 124 tests; the latest signed-build evidence is recorded in [reliability validation](../stream-reliability/validation.md). Individual feature documents record remaining live-validation limits.
...
Latest full suite: **124 tests passed**; signed app build verified. [Live evidence](../stream-reliability/live-validation-2026-09-22.md). The commit containing this status update includes the reliability, keyboard, rolling-FPS and sidebar hit-target changes.

**Next development task: [1080p → 4K MetalFX Spatial first increment](../4k-post-processing/issues/01-spatial-upscaling.md).** Add an optional native scaler with original-scaling bypass, preserve color/aspect ratio, and measure GPU/presentation cost on M1/4K before expanding to sharpening or other backends. This follows the agreed 4K delivery order. Continue reliability soak/long-outage acceptance as separately tracked work; beginning P1 does not mark P0 complete.
