...
The app also has **View → Post-Processing Benchmark…** (Command-Shift-B). It runs bundled-video MetalFX and VideoToolbox workloads without a game, and optionally tests the packaged MLX-DLSS video helper after you choose a local `framegen.safetensors` file. It shows which tested cases have time-budget headroom and exports a JSON report for machine-to-machine comparison. End an active streaming session before starting it. The optional helper is a research video path, not live game frame generation; the selected weights stay outside the app. See the [benchmark design and validation](.scratch/post-processing-benchmark/spec.md) for the workload, timing definitions and limits.
