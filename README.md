...
Stars save title IDs in device-local preferences across restarts. Favorites are not account-synced and do not establish ownership or subscription eligibility; only favorites present in the current account/region catalog appear. Region changes clear the catalog. The new library's query/metadata logic has automated coverage; visual layout and real interaction acceptance remain pending while the development display is asleep. See [the library specification](.scratch/game-library/spec.md).
