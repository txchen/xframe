...
For game input, choose **View → Enable Controller Input** or **View → Enable Keyboard Input** (both can be enabled; the last device with a fresh button press or deliberate stick/trigger motion takes control). Keyboard mode maps physical key positions to the Xbox controller: WASD moves, arrows look, J/Space=A, K=B, U=X, I=Y, Q/E=LB/RB, Z/C=LT/RT, F/T/H/G=D-pad, L/O=stick clicks, Return=Menu and Tab=View. **View → Keyboard Controls…** shows the mapping. Only the focused streaming playback window captures game keys; Command/Control/Option shortcuts remain available. Keyboard sticks/triggers are digital, and custom bindings/mouse look are not implemented. See [keyboard validation](.scratch/keyboard-input/validation.md) for the current live acceptance limits.

## Xbox Account

The main library window opens at startup and restores saved sign-in. When Xbox sign-in is unavailable, it shows the Microsoft sign-in flow in place. Console access and cloud access are checked separately, so lack of a cloud subscription does not hide the Consoles destination. Open account management from the sidebar profile button or **Account → Xbox Account…** (**Shift-Command-A**); it is a sheet in the same window, not a separate account window. Development credential-storage details are collapsed under **Development details**. Video playback retains its dedicated native rendering window.

The rendering window stays hidden until a verified stream video path connects, a local video is opened/replayed, or the test pattern is explicitly requested. Stopping streaming video hides it and returns to the library, where session cleanup status remains visible. Closing local playback stops its decoder and returns to the library. Canceling the local-file picker does not reveal the rendering window.
...
3. XFrame exchanges the authorization for Xbox, xHome, and xCloud credentials, then shows independent console and cloud access, the gamertag, available cloud regions, and credential expiration. This does not yet start a game or prove that every catalog title is playable.
...
## Xbox console streaming (LAN candidate)

Open **Consoles** in the sidebar and choose **Refresh Consoles**. XFrame combines the account device list with the xHome streamable list, then sends a SmartGlass discovery probe on the local IPv4 network. A reported **On** state is shown separately from LAN discovery. Allow XFrame local-network access when macOS prompts; if discovery remains empty, check **System Settings → Privacy & Security → Local Network**. The **Connect** button appears only when the account console ID matches a locally discovered console. For **ConnectedStandby**, choose **Wake Console** first; XFrame sends the dedicated Xbox `Power/WakeUp` command and waits for LAN discovery. Once the console appears, choose **Connect** separately. No game is launched by this action.

A console stream uses the shared H.264, audio, input, and session owner. It requests 1920 × 1080 output and requires the selected WebRTC ICE pair to use host candidates on the same physical IPv4 subnet. Until that path is verified, game audio is muted, video stays hidden, and controller commands are held. A changed, relayed, or unverifiable path ends the session. Brief LAN interruptions use the existing 15-second recovery budget; a longer interruption requires another explicit Connect. **Disconnect** ends XFrame's stream and remote-play session without powering off the Xbox or closing its game. Keep XFrame open until cleanup is confirmed.

Enable controller input from the View menu. Any macOS GameController device exposing an extended gamepad can send input, including Xbox One and DualShock 4 targets. **View → Controller Vibration** and the in-picture Playback Settings panel can turn rumble off without disabling controller input. This preference is saved across app restarts; turning it off stops a playing vibration immediately. Rumble uses the device's exposed haptic actuators; XFrame shows when macOS does not expose them on the current connection. Each incoming vibration command replaces the previous one and uses its transmitted motor levels and duration; the Core Haptics continuous-event limit is 30 seconds. The Series X stream, SmartGlass identity match, selected LAN path, audible audio, physical controller input and rumble, wake, disconnect, and recovery all still require real-console testing. See [the working specification](.scratch/xbox-console-remote-play/spec.md) and [validation record](.scratch/xbox-console-remote-play/validation.md).
...
Open **Account → Cloud Games…** (**Shift-Command-G**), select a game card or list row, and choose **Start Selected Game**. The library loads automatically after sign-in or saved-account restoration; its button shows loading progress and then offers Refresh Games or Retry Loading Games. The account's title list is hydrated with English names, optional poster artwork, categories and publisher from Microsoft's public catalog; it may not be exhaustive, and launch eligibility is ultimately checked by the service.
...
Frame-rate investigation: **Cloud Games → Export frame timing sample…** saves the current bounded timing trace without ending playback. Schema 9 includes RTP timestamps and decoder-input/decode-completion/delivery/actual-presentation events (roughly the last minute). Replay with `python3 scripts/replay-frame-trace.py report.json --output evaluation.json`. This exports numeric timings, not gameplay video; decoder input is after WebRTC jitter buffering. See [capture procedure and limitations](.scratch/dynamic-frame-rate/spec.md). The first [Palworld timing capture and analysis](.scratch/dynamic-frame-rate/validation.md) shows the rate change by decoder input, with 2,536 retained originals and 612 modeled generated frames over 52.46 seconds.

For repeatable 30 → 60 fps cost measurements on a local video, see the [VideoToolbox and MLX-DLSS benchmark](scripts/benchmark-frame-generation/README.md), including M1 results and the M5 run command.

The app also has **View → Post-Processing Benchmark…** (Command-Shift-B). It runs bundled-video MetalFX and VideoToolbox workloads without a game. Import a local `framegen.safetensors` once, or download it from an authorized HTTPS URL, to add the packaged MLX-DLSS video helper; XFrame verifies its SHA-256 and reuses a private local cache on later runs. It shows which tested cases have time-budget headroom and exports a JSON report for machine-to-machine comparison. End an active streaming session before starting it. The optional helper is a research video path, not live game frame generation. See the [benchmark design and validation](.scratch/post-processing-benchmark/spec.md) for the workload, timing definitions and limits.
