...
Enable controller input from the View menu. Any macOS GameController device exposing an extended gamepad can send input, including Xbox One and DualShock 4 targets. Rumble uses the device's exposed haptic actuators; XFrame shows when macOS does not expose them on the current connection. The Series X stream, SmartGlass identity match, selected LAN path, audible audio, physical controller input and rumble, wake, disconnect, and recovery all still require real-console testing. See [the working specification](.scratch/xbox-console-remote-play/spec.md) and [validation record](.scratch/xbox-console-remote-play/validation.md).
